...
/**
 * 최고점수 기록 중 가장 낮은 점수 불러오기
 *
 * @return int lowestScore : 기록 중 가장 낮은 점수
 * */
int getLowestScore()
{
	FILE *fp;
...
/**
 * 이번 게임의 점수를 최고점수 기록에 기록함
 *
 * @param int score : 기록할 이번 게임 점수
 **/
void inputScore(int score)
{
	...
	for(i=0;i<5;i++)
	{
	    if(!fp == '0')  //highscores.txt 파일이 있으면
		    fprintf(fp, "%d\t%d\t\t\t%s\n", i+1, scores[i], highScoreNames[i]);	//파일에 이름과 최고점수들을 기록함
	}
...
void displayHighScores(void) {
	...
	if(fp == NULL) {    //파일이 없는 경우
		createHighScores(); //createHighScores() 함수를 호출해서 파일을 새로 생성
	...
	waitForAnyKey();    //아무 키가 입력 되길 기다리는 함수 호출
	return;
}
